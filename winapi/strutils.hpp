...
extern char* convert_wstr_to_str(WCHAR* wstr);
extern WCHAR* convert_str_to_wstr(char* str);
...
extern WCHAR* read_widestring(void* ptr, size_t buf_sz);
extern vector<string> split_string(char* str, char delim);
extern uint32_t copy_str_to_wstr(char* src, WCHAR* dst, uint32_t str_len);
extern int compare_wstr(char16_t* targ1, char16_t* targ2);
extern uint32_t copy_wstr_to_str(WCHAR* src, char* dst, uint32_t max_len);
